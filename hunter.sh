...
# ====================================================================================
# Aura IP Hunter - v14.1 (自适应 + 配置文件)
# 自动检测网络环境，智能优选 IP 并更新对应 DNS 记录
# ====================================================================================
...
# --- 加载配置文件 ---
# 如果仓库中有 hunter.conf 文件，则加载它来覆盖默认配置
if [ -f "hunter.conf" ]; then
    info "检测到 hunter.conf 配置文件，正在加载自定义设置..."
    source ./hunter.conf
fi
...
# --- 主逻辑 ---
info "启动 Aura IP Hunter v14.1 (自适应 + 配置文件)..."
...
case "$MACHINE_ARCH" in "x86_64") ARCH="amd64" ;; "aarch64") ARCH="arm64" ;; *) error "不支持的架构: ${MACHINE_ARCH}。";; esac
...
for i in "${!TOP_IPS[@]}"; do
    FLEET_MEMBER_INDEX=$((i + 1)); TARGET_DOMAIN="${CF_RECORD_PREFIX_CLEAN}${FLEET_MEMBER_INDEX}.${CF_ZONE_NAME_CLEAN}"; NEW_IP="${TOP_IPS[i]}"
    ...
    if [ -z "$record_info" ]; then
        ...
        if [ "$NEW_IP" == "$CURRENT_IP" ]; then info "  -> IP 未变化，无需更新。"; else
